...
;(function ($) {
    ...
    tojson:    (function() {
      ...
      function s4 (w, ctab0, tab){
        ...
        switch((typeof w).substr(0,3)){
          ...
          case 'num': return isFinite(w) ? '' + (w + '') + '' : 'null';
          case 'boo': case'nul' : return w + '';
          case 'fun': return fj( 
            _cleanFn(w.toString())
      ...
      function _cleanFn(s) {
        var splitter = /\)([\s\n\r\t]+?|\/{1,10}.*?\*\/|\/\/[^\n\r]{0,200}[\n\r]){0,20}?\{/,
            ...
            a0 = a[0],
            isAsync =  a0.to(5)=='async',
            head = a0.from(isAsync ? 14 : 8).replace(/[\s\n\r\t]+?|\/{1,10}.*?\*\/|\/\/[^\n\r]{0,200}[\n\r]/g,'') + ')',
            tail = '{' + s.from(a0.length).replace(splitter,'').replace(/}[^}]+$/,'}');
  ...
  function _unjsonNode (i, node, incl) {
    ...
    if (/^(ob|ar)/.test(t)) _unjson(nd);
      ...
      if (!name.length) e = _NewAsyncFunction(a[2], a[3]);
      else e = _NewAsyncNamedFunction(name, a[2], a[3]);
  ...
  function _NewAsyncFunction (A1, A2) {
    var f;
    try { f = (new Function('', 'return (async function('+A1+'){'+ A2+'});'))(); } 
    catch(e) {  f = { message:e.message, stack:e.stack }; }
    return f;
  }
  
  function _NewAsyncNamedFunction (name, A1, A2) {
    var f;
